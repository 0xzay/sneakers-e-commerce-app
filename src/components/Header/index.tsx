...
export const Header: React.FC<HeaderProps> = ({ usdPrice }) => {
  ...
  const hideScroll = (state: boolean) => {
    if (!state) {
      const styleNode = document.createElement('style');
      styleNode.id = 'scrollbar';
      styleNode.innerText =
        'body::-webkit-scrollbar {width: 0px; background: rgba(255, 255, 255, 0.0);}';
      document.body.appendChild(styleNode);
    } else {
      document.getElementById('scrollbar')?.remove();
    }
  };

  return (
    <header className="flex sticky top-0 w-full items-center pt-8 md:pt-1 pb-1 bg-[#1F1F1F] md:h-24">
        ...
        <div className="flex items-center gap-8">
          <div ref={sortRef}>
            ...
            <ul
              ...
              } z-10 w-56 bg-white border duration-150 transition-all ease-in-out rounded-md`}
            >
          ...
          <input
            type="text"
            className="hidden md:block outline-none w-96 pt-2 pb-2 bg-transparent text-white border-solid border-b opacity-50 p-2 focus:opacity-100 placeholder:text-white bg-[url('././assets/icons/search.svg')] bg-right bg-no-repeat bg-[length:24px_24px]"
            placeholder="Search..."
            ref={inputRef}
          ...
          <div className="flex gap-4">
            <button
              className="rounded-full flex w-[50px] h-[50px] cursor-pointer hover:bg-[#333333] hover:scale-110 active:bg-[#2d2d2d]"
              onClick={() => {
            ...
            >
              ...
              {wish.length > 0 ? (
                <p className="text-[#333333] font-bold text-sm rounded-full absolute ml-7 bg-white pr-2 pl-2 pt-0.5 pb-0.5">
                  {wish.length}
                </p>
              ) : (
                ''
      ...
      >
        ...
        <div className="bg-white text-center border-l w-[350px] md:w-[800px]">
          <h1 className="text-3xl md:text-6xl font-bold p-8 md:p-16 border-b">
            IN CART ({cart.items.length})
          </h1>
          <div className="overflow-auto max-h-[750px]">
            {cart.items.map((obj: any) => (
              <CartProduct key={obj.id} {...obj} usdPrice={usdPrice} />
            ))}
            {cart.items.length > 0 ? (
              <div className="flex flex-col text-2xl p-6">
                <p className="self-end">
                  ...
                  <span className="font-bold">
                    {currency == 'USD' ? '$' : '₽'}
                    {currency == 'USD'
                      ? Math.trunc(cart.totalPrice / usdPrice).toFixed(2)
                      : cart.totalPrice.toFixed(2)}
                  </span>
                </p>
                <button className="p-6 mt-8 uppercase font-bold bg-[#1F1F1F] text-white">
                  checkout
                </button>
              </div>
            ) : (
              <div className="mt-48">
                <svg
                  ...
                  width="150px"
                  height="150px"
                  className="m-auto h-24 w-24 md:h-[150px] md:w-[150px]"
                >
                  <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                  <g
                    id="SVGRepo_tracerCarrier"
                ...
                <p className="md:text-2xl">You have no items in your cart</p>
                <button
                  onClick={() => {
                    setOpenCart(!openCart);
                    hideScroll(openCart);
                  }}
                  className="underline font-bold md:text-xl"
                >
                  View catalog
                </button>
              </div>
      ...
      >
        ...
        <div className="bg-white text-center border-l w-[350px] md:w-[800px]">
          <h1 className="text-3xl md:text-6xl font-bold p-8 md:p-16 border-b">
            IN WISHLIST ({wish.length})
          </h1>
          <div className="overflow-auto max-h-[750px]">
            {wish.map((obj: any) => (
              <WishProduct key={obj.id} {...obj} usdPrice={usdPrice} />
            ))}
            {wish.length === 0 ? (
              <div className="mt-48">
                <svg
                ...
                <p className="md:text-2xl">You have no Saved Items</p>
                <button
                  onClick={() => {
                    setOpenWish(!openWish);
                    hideScroll(openWish);
